...
```
***
在 Vue 和 React 这类前端框架里，“水合”（Hydration）是个关键概念，“水合信息”就和水合过程产生的数据或者状态有关。下面为你详细阐述：

### 水合的概念
在服务器端渲染（SSR）或者静态站点生成（SSG）的场景中，服务端先把 HTML 渲染好，然后发送到客户端。客户端拿到这些 HTML 后，要把它变成可交互的应用程序，这个过程就是水合。简单来说，水合就是把静态的 HTML 和框架里的 JavaScript 代码关联起来，让页面具有交互性。
